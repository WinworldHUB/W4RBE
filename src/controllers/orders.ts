...
export const generateOrderNumber: RequestHandler = async (req, res, next) => {
  try {
    const token = req.headers.authorization?.split(" ")[1];
    const decodedToken: any = jwt.decode(token);
    if (!decodedToken) {
      return res.status(401).json({ message: "please sign in to use this service" });
    }
    ...
    if (!data) {
      return res.status(500).json({ message: "Failed to generate order number, please retry" });
    }
